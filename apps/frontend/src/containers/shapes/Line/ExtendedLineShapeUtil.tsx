...
import { getLineDrawPath, getLineIndicatorPath } from '../shared/getLinePath';
import {
...
import {
  ...
  extendedLineShapeMigrations,
  extendedLineShapeProps,
} from './ExtendedLineShape';
import { getColor, getDash, getStrokeWidth } from '../shared/shape-utils';
import { TLSplineType } from '@tldraw/tlschema';
import { SizeOverride } from '../shared/types';

const handlesCache = new WeakMapCache<ExtendedLineShape['props'], TLHandle[]>();

/** @public */
export class ExtendedLineShapeUtil extends ShapeUtil<ExtendedLineShape> {
  static override type = 'extended-line' as const;
  static override props = extendedLineShapeProps;
  static override migrations = extendedLineShapeMigrations;
  ...
  override getDefaultProps(): ExtendedLineShape['props'] {
    return {
      dash: 'draw',
      size: 'm',
      color: 'red',
  ...
  getGeometry(shape: ExtendedLineShape) {
    console.log(shape);
    // todo: should we have min size?
    return getGeometryForLineShape(shape);
  }

  override getHandles(shape: ExtendedLineShape) {
    console.log(shape);
    return handlesCache.get(shape.props, () => {
      ...
      for (let i = 0; i < spline.segments.length; i++) {
        ...
        const index = getIndexBetween(
          sortedHandles[i].index,
          sortedHandles[i + 1].index
        );
  ...
  override getOutlineSegments(shape: ExtendedLineShape) {
    console.log(shape);
    const spline = this.editor.getShapeGeometry(shape) as
      | Polyline2d
      | CubicSpline2d;
    return spline.segments.map((s) => s.vertices);
  }

  override onResize: TLOnResizeHandler<ExtendedLineShape> = (shape, info) => {
    console.log(shape);
  ...
  override onHandleDrag: TLOnHandleDragHandler<ExtendedLineShape> = (
    shape,
    { handle }
  ) => {
    console.log(shape);
  ...
  override component(shape: ExtendedLineShape) {
    console.log(shape);
    const theme = useDefaultColorTheme();
    const isDarkMode = this.editor.user.getIsDarkMode();
    const spline = getGeometryForLineShape(shape);
    const strokeWidth = getStrokeWidth(
      shape.props.size as ExtendedLineSizeOptions,
      shape.props.opts
    );
    const color = getColor(shape.props.color, isDarkMode, shape.props.opts);
    const dash = getDash(shape.props.dash, shape.props.opts);
    ...
    if (shape.props.spline === 'line') {
      ...
      if (dash === 'draw') {
        const outline = spline.points;
        const [innerPathData, outerPathData] = getDrawLinePathData(
          shape.id,
          outline,
          strokeWidth
        );
    ...
    // return <SVGContainer id={shape.id}></SVGContainer>;
  }

  indicator(shape: ExtendedLineShape) {
    console.log(shape);
    const strokeWidth = getStrokeWidth(
      shape.props.size as ExtendedLineSizeOptions,
      shape.props.opts
  ...
  override toSvg(shape: ExtendedLineShape) {
    console.log(shape);
    ...
    switch (dash) {
      ...
      default: {
        ...
        const fn =
          spline instanceof CubicSpline2d
            ? getSvgPathForBezierCurve
            : getSvgPathForEdge;
