import { ExtendedLineDashOptions } from '../Line/ExtendedLineShape'
import { LayerOrBlockOptions } from './types'
// import { DashOverride } from './types'

export function getPerfectDashProps(
	totalLength: number,
	strokeWidth: number,
	opts = {} as Partial<{
		style: ExtendedLineDashOptions
		snap: number
	...
	layerOrBlock?: LayerOrBlockOptions
): {
	strokeDasharray: string
	strokeDashoffset: string
} {
	if (opts.style === 'ByLayer' && layerOrBlock?.layer) {
		return getPerfectDashProps(totalLength, strokeWidth, { ...opts, style: layerOrBlock.layer.dash })
	} else if (opts.style === 'ByBlock' && layerOrBlock?.block) {
		return getPerfectDashProps(totalLength, strokeWidth, { ...opts, style: layerOrBlock.block.dash })
	}
	const {
		closed = false,
		snap = 1,
