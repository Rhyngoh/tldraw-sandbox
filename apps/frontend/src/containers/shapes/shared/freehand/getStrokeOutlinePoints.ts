...
export function getStrokeOutlinePoints(
  ...
  */
	...
	if (taperStart || (taperEnd && strokePoints.length === 1)) {
		...
		const cornersvec2d = Vec.Sub(leftPts[0], rightPts[0])
		const offsetA = Vec.Mul(cornersvec2d, 0.5)
		const offsetB = Vec.Mul(cornersvec2d, 0.51)
